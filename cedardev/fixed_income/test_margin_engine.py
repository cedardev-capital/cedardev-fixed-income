...
def sample_positions(as_of_date):
    ...
    rows = [
        ...
        (0.5,  "LONG",  100.0,  1_000_000, "CDC-RATES-001"),   # 0-2
        (1.5,  "SHORT", -80.0,    900_000, "CDC-RATES-001"),   # 0-2
        (3.0,  "LONG",  500.0,  5_000_000, "CDC-RATES-001"),   # 2-5
        (4.5,  "SHORT", -300.0, 3_000_000, "CDC-RATES-001"),   # 2-5
        (7.0,  "LONG",  900.0,  9_000_000, "CDC-RATES-001"),   # 5-10
        (7.0,  "SHORT", -400.0, 4_000_000, "CDC-RATES-001"),   # 5-10
        (15.0, "LONG",  700.0,  7_000_000, "CDC-RATES-001"),   # 10-20
        (25.0, "SHORT", -200.0, 2_000_000, "CDC-RATES-001"),   # 20-30
        (32.0, "LONG",  1500.0,15_000_000, "CDC-RATES-001"),   # 30+
    ]
    records = []
...
class TestIntraBucketNetter:
    ...
    def test_net_dv01_sum(self, categorizer, sample_positions):
        ...
        row = netting_df[
            (netting_df["portfolio_id"] == "CDC-RATES-001") &
...
class TestCrossMarginEngine:
    ...
    def test_no_credit_for_same_direction_buckets(self, as_of_date):
        ...
        """
        rows = [
            (3.0, "LONG", 500.0, 5_000_000, "CDC-RATES-001"),
            (7.0, "LONG", 900.0, 9_000_000, "CDC-RATES-001"),
        ]
        records = []
        for i, (yrs, side, dv01, mv, portf) in enumerate(rows):
            records.append({
